...
class model(object):
    ...
    def calc_f(self, transfer_instance, **DarkOptions):
        ...
        if 1==0:
            pass
        #not isinstance(transfer_instance, transfer):
...
class annihilating_spikes_model(model):
    def __init__(self,ref_el_spec,ref_ph_spec,ref_oth_spec, m, MPBH,fPBH, sigmav,spikeprofiles,logEnergies=None,redshift=None, **DarkOptions):

        from .special_functions import boost_factor_spikes_from_file, boost_factor_spikes_vec

    ######
        #print_info("I am annihilating_spikes_model")

        def scaling_boost_factor(redshift, spec_point, MPBH,fPBH, m, sigmav, spikeprofiles):
            #ret =  spec_point*boost_factor_spikes(redshift, MPBH, fPBH, m*1e-9, sigmav, spikeprofiles)  #reconvert  DM mass in GeV
            ret =  spec_point*boost_factor_spikes_from_file(redshift, MPBH, fPBH, m*1e-9, sigmav, spikeprofiles)
            return ret
        ...
        #normalization /= boost_factor_spikes_vec(redshift, MPBH, fPBH, m*1e-9, sigmav, spikeprofiles)
        normalization /=  np.vectorize(boost_factor_spikes_from_file).__call__(redshift, MPBH, fPBH, m*1e-9, sigmav, spikeprofiles)

        spec_electrons = np.vectorize(scaling_boost_factor).__call__(redshift[None,:],ref_el_spec[:,None],MPBH,fPBH, m, sigmav, spikeprofiles)
        spec_photons = np.vectorize(scaling_boost_factor).__call__(redshift[None,:],ref_ph_spec[:,None],MPBH,fPBH, m, sigmav, spikeprofiles)

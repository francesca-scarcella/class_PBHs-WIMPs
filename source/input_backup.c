...
int input_read_parameters(
  ...
  */
  ...
  class_read_double("PBH_spiked_mass",pth->PBH_spiked_mass);  //FRANCESCA
  class_read_double("PBH_fraction",pth->PBH_fraction);
  //printf("Hello 5, PBH_spiked_mass is %d\n",pth->PBH_spiked_mass);
  //printf("pth->PBH_spiked_mass  %e\n",pth->PBH_spiked_mass );
//  printf("Hello 3, DM_mass is %d\n",pth->DM_mass);
  ...
  if(pth->PBH_accreting_mass>0. && pth->PBH_fraction>0){
    ...
    //  printf("pth->PBH_evaporating_mass %e\n",pth->PBH_evaporating_mass);
    //  printf("pth->PBH_evaporating_mass >0  %e\n",pth->PBH_evaporating_mass > 0.);
    //  printf("pth->PBH_spiked_mass  %e\n",pth->PBH_spiked_mass );
    //  printf("pth->DM_mass >0  %e\n",pth->DM_mass );

 //  class_test(pth->PBH_evaporating_mass < 1e15 && pth->PBH_fraction > 1e-4,errmsg,  //FRANCESCA: revoming this error, pth->PBH_evaporating_mass  should be zero...?
  ...
  class_test(pth->PBH_fraction>0. && (pth->PBH_accreting_mass==0. && pth->PBH_evaporating_mass==0. && pth->PBH_spiked_mass==0. ),errmsg,
    "Hey!You have asked for a fraction of PBH being DM but you have 'PBH_accreting_mass == 0 && PBH_evaporating_mass ==0'. Please choose a value (in Msun for PBH_accreting_mass, in g for PBH_evaporating_mass).");  //FRANCESCA
  ...
  /*class_test(pth->PBH_fraction>0 && pth->DM_mass>0,errmsg,
    ...
    /*FRANCESCA: this error should be removed?*/
    ...
    if (flag1 == _TRUE_){
      ...
      if (strcmp(string1,"from_file") == 0) {
        class_call(parser_read_string(pfc,"J_factors_file",&string1,&flag1,errmsg),
                   ...
                   errmsg);
        if (flag1 == _TRUE_){
          class_read_string("J_factors_file",pth->J_factors_spike_file);
        }
        else {
          class_test(flag1==_FALSE_,errmsg,
            "You have forgotten to specify the file to the J-factors. ");
        }
        flag2=_TRUE_;
      }
      class_test(flag2==_FALSE_,errmsg,
  ...
  if(pth->annihilation>0. || pth->decay_fraction>0. || pth->PBH_accreting_mass > 0. || pth->PBH_evaporating_mass > 0. || pth->PBH_spiked_mass > 0.){
 ...
 (choose from 'annihilation', 'annihilation_halos', 'decay', 'evaporating_PBH', 'accreting_PBH')
        ...
        /* Automatic comand for annihialtion with halo boost*/
          ...
          printf("calling Dark Ages with hist=annihilation_halos  ");
        ...
        /** - Automatic command for energy injection form DM spikes around PBHs */
          ...
          printf("calling Dark Ages with hist=annihilation_spikes ");

          strcat(ppr->command_fz,"/DarkAgesModule/bin/DarkAges --hist=annihilation_spikes --spectrum ");
          sprintf(string2,"");
          class_call(parser_read_string(pfc,"injected_particle_spectra",&string2,&flag1,errmsg),
                     errmsg,
                     errmsg);
          ...
          sprintf(string2,"%g",pth->DM_mass);
          strcat(ppr->command_fz,string2);
          strcat(ppr->command_fz," --fPBH=");
          sprintf(string2,"%g",pth->PBH_fraction);
          strcat(ppr->command_fz,string2);
          strcat(ppr->command_fz," --PBH_mass=");
          sprintf(string2,"%g",pth->PBH_spiked_mass);
          strcat(ppr->command_fz,string2);
          strcat(ppr->command_fz," --sigmav=");
          sprintf(string2,"%g",pth->annihilation_cross_section);
          strcat(ppr->command_fz,string2);
          strcat(ppr->command_fz," --spikeprofiles=");
          sprintf(string2,"%d",pth->PBH_spike_profiles);
          printf("I'm calling DarkAges and giving spikeprofiles= %d\n", pth->PBH_spike_profiles);
          strcat(ppr->command_fz,string2);
          printf("done!");
...
int input_default_params(
  ...
  pth->annihilation_z_halo = 30; //dummy number to ignore this by default
  pth->has_on_the_spot = _TRUE_;
  pth->print_energy_deposition_function = _FALSE_;
